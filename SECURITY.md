...
### v1.8.0 (2026-08-07)
- ✅ 身份画像系统：多中心画像 + shadow clustering + rescue 模式 + 校准报告 + ANN 索引快照/增量
- ✅ 人物列表/照片管理列表虚拟滚动 + 连续浏览
- ✅ 人脸排除（持久化误检/低质量样本）
- ✅ 照片详情页单张人脸改名/归属/拆分
- ✅ 人物详情页密度切换 + 隐藏/恢复 + Tab 布局 + 虚拟网格
- ✅ 后台任务协调器：负载采样节流 + 压力状态展示 + SQLite busy 上报
- ✅ Analyzer 故障分类 + 熔断器 + 持久化重试 + durable lease release
- ✅ 重复人脸审计 CLI（只读 P0）
- ✅ NAS 备份 worker（SSH 原子备份）
- ✅ 人物详情页性能重构（虚拟网格 + count-free cursor 分页 + 派生表加速）
- ✅ Dashboard 查询性能优化（消除冗余全表 COUNT）
- ✅ embedding 0x5B 误判 JSON、拆分归属冲突、move/split 幂等化等多项修复
