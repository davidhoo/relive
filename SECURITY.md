...
- ✅ 设置合理的过期时间
- ✅ 定期审查和清理未使用的 Key
- ⚠️ 通过 Header 传递（当前支持 URL 参数，计划在 v0.3.0 移除）
...
1. 使用反向代理添加 IP 白名单
2. 限制只从内网访问
3. 等待 v0.3.0 版本（计划添加认证）

**临时解决方案（Nginx）**：
```nginx
location ~ ^/api/v1/photos/\d+/(image|thumbnail)$ {
    # 只允许内网访问
...
| 问题 | 严重性 | 状态 | 计划版本 |
|------|--------|------|----------|
| 照片 API 无认证 | 🔴 高 | 已知 | v0.3.0 |
| API Key 可通过 URL 传递 | 🟠 中 | 已知 | v0.3.0 |
| 容器以 root 运行 | 🟡 低 | 已知 | v0.3.0 |
| 缺少请求频率限制 | 🟡 低 | 已知 | v0.3.0 |
| CORS 包含开发环境域名 | 🟡 低 | 已知 | v0.3.0 |
